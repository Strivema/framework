...
<script type="text/javascript">
    ...
    function loadData() {
        ...
        var method = "";
        if (id !== "") {
            dataForm.beeForm("/sys/menu/" + id, function (result) {
                $("#parentName").val(result.data.parentName);
                menuSelect.attr("data-url", "sys/menu/jstree/?disableId=" + result.data.id + "&&selectedId=" + result.data.parentId);
                menuSelect.beeJsTree();
            ...
            method = "POST";
            menuSelect.attr("data-url", "sys/menu/jstree/");
            menuSelect.beeJsTree();
        }
    ...
    function loadSimpleLineIcons() {
        $.getJSON("json/simple-line-icons.json", function (data) {
            $("select[name='icon']").html("");
            $.each(data, function (i, n) {
                $("select[name='icon']").append('<option value="' + n + '" data-icon="' + n + '">' + n + '</option>')
