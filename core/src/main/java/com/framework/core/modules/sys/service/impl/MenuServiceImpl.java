...
public class MenuServiceImpl extends ServiceImpl<MenuMapper, Menu> implements MenuService {
    ...
    public Integer getMaxSort(Integer parentId){
        return baseMapper.getMaxSort(parentId);
    }
    /**
     * 通过url获取层级url,用于面包屑导航
     *
     * @param href
     * @return
     */
    @Override
    public List<Menu> getMenuByUrl(String href) {
        List<Menu> list = new ArrayList<>();
        List<Menu> menus = baseMapper.selectByMap(ImmutableMap.of("href", href));
        if (menus.size() == 0) {
            return null;
        }
        ...
        // 将数组倒叙
        Collections.reverse(Arrays.asList(parentIdArr));
        for (String parentId : parentIdArr) {
            ...
            if ("0".equals(parentId)) {
                Menu home = new Menu();
                home.setHref(Const.MENU_HOME_HREF);
                home.setName(Const.MENU_HOME_NAME);
                home.setIcon(Const.MENU_HOME_ICON);
                list.add(home);
                continue;
            }
